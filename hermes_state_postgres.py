...
class PostgresSessionDB:
        ...
        with self._pool.connection() as conn:
            with conn.transaction():
                ...
                conn.execute(
                    ...
                    f"ADD COLUMN IF NOT EXISTS platform_message_id TEXT"
                )
                conn.execute(
                    f"""
                ...
                conn.execute(
                    f"CREATE UNIQUE INDEX IF NOT EXISTS idx_hermes_messages_platform_id "
                    f"ON {messages}(session_id, platform_message_id) "
        ...
        for row in rows:
            ...
            if msg.get("tool_calls"):
                try:
                    ...
                    msg["tool_calls"] = []
            if msg.get("platform_message_id"):
                msg["message_id"] = msg["platform_message_id"]
            result.append(msg)
        return result
    ...
    def get_messages_around(
        ...
        if window < 0:
            window = 0
        messages = sorted(self.get_messages(session_id), key=lambda msg: int(msg.get("id") or 0))
        anchor_index = next(
            (idx for idx, msg in enumerate(messages) if int(msg.get("id") or 0) == int(around_message_id)),
            -1,
        )
        if anchor_index < 0:
        ...
        return {
            ...
            "messages_before": anchor_index - start,
            "messages_after": end - anchor_index - 1,
        }
    ...
    def get_anchored_view(
        ...
        if keep_roles is not None:
            ...
            filtered_window = [
                msg
                for msg in window_rows
                if int(msg.get("id") or 0) == int(around_message_id) or msg.get("role") in keep_set
            ]
        else:
            filtered_window = window_rows

        all_messages = sorted(self.get_messages(session_id), key=lambda msg: int(msg.get("id") or 0))
        window_min_id = int(window_rows[0].get("id") or 0)
        window_max_id = int(window_rows[-1].get("id") or 0)

        def _bookend_candidate(msg: Dict[str, Any]) -> bool:
            if keep_roles is not None and msg.get("role") not in set(keep_roles):
                return False
        ...
        start_candidates = [
            ...
            if int(msg.get("id") or 0) < window_min_id and _bookend_candidate(msg)
        ]
        end_candidates = [
            msg for msg in all_messages
            if int(msg.get("id") or 0) > window_max_id and _bookend_candidate(msg)
        ]
        return {
            "window": filtered_window,
            "messages_before": primitive["messages_before"],
            "messages_after": primitive["messages_after"],
            "bookend_start": start_candidates[:bookend],
            "bookend_end": end_candidates[-bookend:] if bookend else [],
        }
    ...
    def search_messages(
        ...
        del sort  # Postgres hybrid search ranks by lexical/vector score.
    ...
    def request_handoff(self, session_id: str, platform: str) -> bool:
        ...
        return self._execute_write(_do, lock_key=session_id)
